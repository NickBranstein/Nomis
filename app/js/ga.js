...
`makeInteractive`: Assigns `press` and `g` actions to sprites and adds pointer interactivity.
...
GA.create = function(width, height, setup, assetsToLoad, load) {
  ...
  */
  ...
  ga.canvas = document.getElementById("canvas");
  ...
  function update() {
    ...
    if (ga.buttons.length > 0) {
      ...
      for(var i = ga.buttons.length - 1; i >= 0; i--) {
        ...
        if(button != undefined && button.update != undefined)
        {
...
//   };
  ...
  ga.circle = function(diameter, fillStyle, strokeStyle, lineWidth, x, y) {
    ...
    // TODO: need to fix the glow effect me thinks
        o.updateFlash = function() {
          if(o.flash){
                ...
                o.strokeStyle = "rgba(" + constants.colors[0] + "," + o.alpha + ")";
                o.fillStyle = "rgba(" + constants.colors[0] + "," + o.alpha + ")";
            }
            else {
                o.alpha = 0.5;
            }
...
//   };
  ...
  ga.addStatePlayer = function(sprite) {
    ...
    function playSequence(sequenceArray) {
      ...
      endFrame = sequenceArray[sequenceArray.length - 1]; //this is a ga bug
      numberOfFrames = sequenceArray.length - 1;//endFrame - startFrame;
    ...
    function advanceFrame() {
      ...
      if (frameCounter < numberOfFrames) {
        ...
        if (sprite.loop) {
          sprite.gotoAndStop(startFrame);
          frameCounter = 0; // another bug shouldn't this be set to 0??
        }
      }
    }
...
//     },
  ...
  function makePointer(){
    ...
    o.hitTestSprite = function(sprite) {
      ...
      // is the sprite triangular
      if(sprite.triangular)
      {
        hit = ga.canvas.getContext('2d').isPointInPath(sprite.triangle, o.x, o.y);
      }
      //Is the sprite rectangular?
      else if (!sprite.circular) {
